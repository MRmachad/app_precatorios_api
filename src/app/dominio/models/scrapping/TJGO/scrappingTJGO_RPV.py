...
class scrappingTJGO_RPV(BaseScrapping):
    ...
    def adicione_processo_em_rastreamento(self, numero:str):        
        if scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA and len(scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA) < 100000 :            
            scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA.append(numero) 
        elif scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA:
            scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA.pop()
            scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA.append(numero) 
        else:
            scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA = [numero]
    ...
    async def work(self) -> Any:
        try:        
            ...
            primeiro_inexistente = await self.servicoDeProcesso.obtenha_data_primeiro_inexistente() if not scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA else await self.servicoDeProcesso.obtenha_data_primeiro_inexistente(scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA)
            ...
            if(primeiro_inexistente is not None):
                ...
                while True:                    
                    ...
                    if await self.findAndInsert(metaProcesso=inexistente):
                        scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA.remove(inexistente.NumeroProcesso)
                    inexistente = await self.servicoDeProcesso.obtenha_data_primeiro_inexistente(scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA)
                    if(inexistente is None):
                        break
            elif scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA:
                while scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA:
                    try:
                        inexistente = await self.servicoDeMetaProcesso.obtenha_por_numeroProcesso(numero_processo=scrappingTJGO_RPV.PROCESSO_EM_RASTREAMENTO_OU_FALHA.pop())
                        ...
                        log_error(e)
                        print(f"Erro no worker scrappingTJGO_RPV, falha ao tratar processo em falha ou rastreamento")  

        except Exception as e:
            log_error(e)
            print(f"Erro no worker scrappingTJGO_RPV")  
